...
@RequestMapping("/api/training/charts/{chartId}/risk-rule")
public class TrainingRiskRuleController {
    ...
    /**
     * 리스크 규칙 조회
     * GET /api/training/charts/{chartId}/risk-rule
     *chartId
    ...
    public ResponseEntity<RiskRuleResponse> get(
            ...
            @PathVariable Long chartId       // 조회할 훈련 chart ID
    ) {
        Object p = authentication.getPrincipal();
        Long userId = (p instanceof Long) ? (Long) p : Long.valueOf(p.toString());
        return ResponseEntity.ok(service.get(userId, chartId));
    }

    /**
     * 리스크 규칙 생성/수정 (Upsert)
     * PUT /api/training/charts/{chartId}/risk-rule
     *
     * - 기존 룰이 있으면 수정, 없으면 생성
    ...
    public ResponseEntity<RiskRuleResponse> upsert(
            Authentication authentication,          // 로그인 사용자(Principal)
            @PathVariable Long chartId,             // 대상 훈련 chart ID
            @Valid @RequestBody RiskRuleUpsertRequest req // 요청 DTO (검증 적용)
    ) {
        Object p = authentication.getPrincipal();
        Long userId = (p instanceof Long) ? (Long) p : Long.valueOf(p.toString());
        return ResponseEntity.ok(service.upsert(userId, chartId, req));
    }
}
