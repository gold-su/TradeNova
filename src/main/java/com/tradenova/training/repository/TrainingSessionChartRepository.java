...
public interface TrainingSessionChartRepository extends JpaRepository<TrainingSessionChart, Long> {
    ...
    @Query("select c from TrainingSessionChart c where c.session.id = :sessionId order by c.id asc")
    List<TrainingSessionChart> findAllForUpdateBySessionIdOrderByIdAsc(
